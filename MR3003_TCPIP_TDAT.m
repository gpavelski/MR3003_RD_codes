...
TXSet = [0,-45;1,-25; 2,-15; 4,-10; 7,-5; 11,0; 20,5; 37,10; 63,15];
 ...
 k = 1; % Loop counter
 np = 200; % Number of packages received
 global h ;
 
r_t = zeros(32,1);  % Range vector
...
axis manual %// this line freezes the axes
 ...
 RXGain = 14; % Receiver Gain
 TXAN = 0; % Transmitter antenna number
 RSSR = 0; % Range and Speed Settings
...
% Transmit data to the server (or a request for data from the server). 
 ...
 if Mact(6) == 0    % If TDAT is off, then turn it on
      RModeE = char(sscanf(['44534631' '00000004' MSG{6}],'%2X').'); %DSF1 Enable TDAT
      fwrite(t,RModeE);
 end
 ...
 while flg ~= 1
     ...
     v(1:len/2,1) =  256*r(1:2:end-1,1) + r(2:2:end,1); 
         ...
         st = find(v == 21572); % Positions relative to start of each frame
