...
filename = 'Record_2019-03-06_15-52-27 - Beidou07';
w = strsplit(filename, ' - ');
fname = w{1};
posname = w{2};
...
[T,theta,ax,ay,az] = GetPositionIMU(fname,posname,firstFrame);
ax(length(df)+1:end) = [];
ay(length(df)+1:end) = [];
az(length(df)+1:end) = [];
T(length(df)+2:end) = [];
...
R = sqrt(ax.^2+ay.^2);
A = atan2d(ay,ax);
comp = linspace(0,-length(df)*0.0197,length(df));
dir = theta(1:length(df));%+comp';
...
cell_wid = 12.5;%6.25; % Cell width in centimeters
